...
(accounts.views.exchange_code_for_claims, imported from accounts.google_oauth) is
...
Google's API. accounts/tests/test_google_oauth_mechanics.py-equivalent coverage
...
from accounts.google_oauth import GoogleOAuthError, exchange_code_for_claims
...
class GoogleOAuthMechanicsTests(TestCase):
    """Unit tests for accounts/google_oauth.py's exchange_code_for_claims, with
    ...
    @patch('accounts.google_oauth.google_id_token.verify_oauth2_token')
...
class GoogleCallbackViewTests(TestCase):
    ...
    def test_state_is_single_use(self):
        state = self._start()
        with patch('accounts.views.exchange_code_for_claims', return_value=make_claims()):
    ...
    def test_invalid_or_expired_token_shows_friendly_error(self):
        state = self._start()
        with patch('accounts.views.exchange_code_for_claims', side_effect=GoogleOAuthError('bad token')):
            with self.assertLogs('accounts.views', level='WARNING'):
    ...
    def test_unverified_email_is_rejected(self):
        ...
        with patch('accounts.views.exchange_code_for_claims', return_value=claims):
            response = self.client.get(reverse('google_callback'), {'state': state, 'code': 'abc'})
        self.assertRedirects(response, reverse('login'))
